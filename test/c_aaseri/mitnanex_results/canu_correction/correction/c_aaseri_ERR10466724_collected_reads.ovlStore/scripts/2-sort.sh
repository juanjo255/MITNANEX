...
#  This script should be executed from correction/c_aaseri_ERR10466724_collected_reads.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./c_aaseri_ERR10466724_collected_reads.ovlStore.BUILDING/$jobname.info -a ! -e ./c_aaseri_ERR10466724_collected_reads.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './c_aaseri_ERR10466724_collected_reads.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./c_aaseri_ERR10466724_collected_reads.ovlStore.BUILDING \
  -S ../c_aaseri_ERR10466724_collected_reads.seqStore \
  -C  ./c_aaseri_ERR10466724_collected_reads.ovlStore.config \
  -f \
  -s $jobid \
  -M 1 

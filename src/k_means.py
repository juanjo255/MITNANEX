...
reads_per_cluster_gc = pd.read_csv(
    "gc_content_per_cluster.tsv", delimiter="\t", header=None
)
...
kmeans = KMeans(
    n_clusters=20, max_iter=100, init="k-means++", random_state=0, n_init=1, verbose=1
)
prediction = kmeans.fit_predict(
    reads_per_cluster_gc.loc[:, ["covertura", "gc_content"]]
)


plt.scatter(
    x=reads_per_cluster_gc["gc_content"],
    y=reads_per_cluster_gc["covertura"],
    c=prediction,
)

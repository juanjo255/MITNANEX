...
def assign_cluster(
    alignment: psa,
    cluster_pointers: hash_table_ids,
    ...
    if query_pointer and reference_pointer:
        ...
        # If two reads in different cluster, save the read in the cluster of the longest read in the aligment  # noqa: E501
        ...
        #         longest_read_cluster.update_longest_read(
        #             longest_read_length, longest_read_id
        #         )
    elif query_pointer:
        ...
        if query_cluster.longest_read_length < alignment.reference_length:
            query_cluster.update_longest_read(
                alignment.reference_length, alignment.reference_id
            )
        ...
        if reference_cluster.longest_read_length < alignment.query_length:
            reference_cluster.update_longest_read(
                alignment.query_length, alignment.query_id
            )
